    ...
    export default {
        ...
        try {
            ...
            if (!currentUser) {
                ...
                // Bạn có thể chuyển hướng đến trang đăng nhập hoặc hiển thị một modal đăng nhập ở đây
                this.$router.push('/auth/login');
                return;
            }

            if (!this.book || !this.book.id) {
                console.error("ID sách không tồn tại.");
                return;
            }

            // Nếu người dùng đã đăng nhập, tiến hành thêm sản phẩm vào giỏ hàng
            const result = await CartService.addToCart(this.book.id, this.quantity);
            console.log("Sản phẩm đã thêm vào giỏ:", result);
    ...
    .details-list {
        display: grid;
        grid-template-columns: 1fr 1fr; /* Chia thành hai cột đều nhau */
    ...
    .details-list .label {
        ...
        margin-right: 8px; /* Khoảng cách nhỏ giữa nhãn và giá trị */
    }
    ...
    .action-container {
        display: flex;
        align-items: center;
        gap: 20px; /* Khoảng cách giữa nút số lượng và nút Chọn Mua */
    }
    .quantity-selector {
        display: flex;
        align-items: center;
